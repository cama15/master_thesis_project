...
# Load the dataframe
file_path = "/data/cephfs-1/work/groups/kühnen/users/cama15_c/cpg10_mlome_filtered_df.csv"
...
axes[0].set_title("Methylation and Sequencing Coverage at the Hypomethylated Control CpG 10 Region")
...
for i, state in enumerate(cell_states):
    ...
    methylation = subset["Meth_fracton"]
    ...
    if i == num_plots - 1:
        ax1.set_xlabel("Genomic Position (chr1:26,865,482-chr1:26,865,486)")
        ax1.set_xticks([26865482, 26865484])
        ax1.set_xticklabels(["26,865,482", "26,865,484"])

# Adjust layout to prevent overlap
fig.tight_layout()

# Save the individual plots as a single JPEG
individual_plots_filename = os.path.join(output_dir, "cpg10_by_cell_state_methylation_coverage.jpeg")
...
xticks = [26865482, 26865484
    ]

xticklabels = [
    "26,865,482", "26,865,484"
]

for state in cell_states:
    subset = df[df["Cell_state"] == state].dropna(subset=["Meth_fracton"])
...
ax_combined.set_xlabel("Genomic Position (chr1:26,865,482-chr1:26,865,486))")
...
ax_combined.set_title("Methylation and Sequencing Coverage at the Hypomethylated Control CpG 10 Region")
ax_combined.legend(loc="center left", bbox_to_anchor=(1.0, 0.5))

# Save combined plot
combined_filename = os.path.join(output_dir, "cpg10_methylation_lines.jpeg")
