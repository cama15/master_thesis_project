...
```{r}
data <- read_csv("simple_pax_pyroseq.csv")
```
...
```{r}
df_new <- df_long %>%
  filter(!Sample_ID %in% c("S38", "NK", "T10", "S95", "X61", "X69"))
print(df_new)
```
```{r}
naive_mean <- df_new %>%
  filter(Cell_State == "Naive") %>%
  summarise(mean_meth = mean(Methylation_Percentage, na.rm = TRUE))
...
# Step 5: Compute max methylation for label placement
max_y_values <- df_filtered %>%
  group_by(Cell_State) %>%
  summarise(max_meth = max(Methylation_Percentage, na.rm = TRUE), .groups = "drop")

# Merge the mean and max values
mean_labels <- left_join(mean_labels, max_y_values, by = "Cell_State")

# Step 6: Adjust mean label positions to avoid exceeding the y-axis range (0 to 100)
mean_labels <- mean_labels %>%
  mutate(y_label_position = pmin(max_meth + 5, 100))  # Clip the label position to 100

# Step 7: Boxplot with mean labels and custom legend
p <- ggplot(df_filtered, aes(x = Cell_State, y = Methylation_Percentage, fill = Track)) +
  geom_boxplot(width = 0.6, outlier.shape = NA) +
  geom_jitter(width = 0.15, size = 1, alpha = 0.4) +
  geom_text(data = mean_labels, aes(x = Cell_State, y = y_label_position,
...
ggsave("avg_meth_boxplot_with_means_updated.png", plot = p, width = 12, height = 8, dpi = 300)

# Step 9: Print the plot
print(p)
```
